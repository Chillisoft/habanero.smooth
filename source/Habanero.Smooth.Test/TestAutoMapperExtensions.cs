...
#region ManyToOneReverseRelationship
        ...
        {
            //---------------Set up test pack-------------------
            PropertyInfo info = RandomValueGenerator.GetMockPropInfoWithNoAutoMapProp<AutoMapOneToMany>();
        ...
        {
            //---------------Set up test pack-------------------
            PropertyInfo info = RandomValueGenerator.GetMockPropInfoWithAutoMapAttribute<AutoMapOneToMany>();
            PropertyWrapper propertyWrapper = info.ToPropertyWrapper();
            object[] customAttributes = info.GetCustomAttributes(typeof(AutoMapOneToMany), true);
            //---------------Assert Precondition----------------
            Assert.IsNotNull(customAttributes);
        ...
        public void Test_GetMappedReverseRelationshipName_WhenAutoMapPropWithReverseRel_ShouldReturnReverseRel()
        {
            ...
            PropertyInfo info = RandomValueGenerator.GetMockPropInfoWithAutoMapAttribute<AutoMapOneToMany>(expectedRevRelName);
            PropertyWrapper propertyWrapper = info.ToPropertyWrapper();
            object[] customAttributes = info.GetCustomAttributes(typeof(AutoMapOneToMany), true);
            //---------------Assert Precondition----------------
            Assert.IsNotNull(customAttributes);
        ...
        public void Test_GetSingleRevRelationshipName_WhenNoRevRel_WhenHasAttribute_WhenHasRevRelName__ShouldBeRevRelName()
        {
            ...
            //---------------Assert Precondition----------------
            relPropInfo.AssertHasAttribute<AutoMapOneToOne>(attribute => attribute.ReverseRelationshipName == mappedRevRelName);
            //---------------Execute Test ----------------------
